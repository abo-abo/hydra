...
this function is designed for heads without a property :column
"
  (let (alist)
    (dolist (h heads)
...
(defun hydra--normalize-heads (heads)
  "ensure each head from HEADS have a property column or set it to the same value as preceding head or nil if no previous value
was defined"
  (let ((current-col nil))
    (mapcar (lambda (head)
